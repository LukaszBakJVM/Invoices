...
public class InvoicesController {
    ...
    @PostMapping
    ResponseEntity<Void> createInvoices(@RequestBody InvoicesDtoRequest request) {
        invoicesService.createInvoices(request);
    ...
    ResponseEntity<InvoicesNumber> invoicesNumber(@AuthenticationPrincipal User user) {
        return ResponseEntity.ok(invoicesService.invoicesNumber(user.getEmail()));
    }
